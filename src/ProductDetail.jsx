...
function ProductDetail({ products }) {
  ...
  useEffect(() => {
    ...
    fetch('http://192.168.5.119:3001/api/prod-process')
      .then(res => res.json())
      .then(data => setProcessStages(data))
      .catch(() => setProcessStages([]));
  }, []);

  if (!products || products.length === 0) return <div className="p-8">Đang tải...</div>;
  const product = products.find(p => p.index === index);
  if (!product) return <div className="p-8 text-red-600">Không tìm thấy sản phẩm!</div>;
