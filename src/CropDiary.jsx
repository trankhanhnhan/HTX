...
function CropDiary({ onSelectBatchCode }) {
  ...
  // Đọc danh sách sản phẩm đã thêm từ productTypes.csv (giả sử backend trả về API)
  useEffect(() => {
    ...
    fetch('http://192.168.5.119:3001/api/plots')
    .then(res => res.json())
    .then(data => {
      setPlotCodes(data.map(plot => plot.batchCode));
      setPlotCodesWithArea(data); // data là mảng các object có batchCode, area, areaFree
    });
  }, []);
  ...
  if (areaHa > areaFree) {
    ...
    try {
      ...
      let url = 'http://192.168.5.119:3001/api/crop-diaries';
      let method = 'POST';

      if (editMode) {
        url = `http://192.168.5.119:3001/api/crop-diaries/${form.index}`;
        method = 'PUT';
      }
      ...
      if (data.success) {
        ...
        // Reload diary list
        const response = await fetch('http://192.168.5.119:3001/api/crop-diaries');
        const diaries = await response.json();
        setDiaryList(diaries);

        // Reload plots để lấy areaFree mới nhất
        const plotsRes = await fetch('http://192.168.5.119:3001/api/plots');
        ...
        await fetch('http://192.168.5.119:3001/api/prod-process', {
          method: 'POST',
          body: formData,
        });
  ...
  if (!form.productId || !form.name || !form.season || 
    ...
    try {
      ...
      // Send update request
      const res = await fetch(`http://192.168.5.119:3001/api/crop-diaries/${form.index}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      if (data.success) {
        ...
        if (completedDiary) {
          ...
          const completeRes = await fetch(`http://192.168.5.119:3001/api/complete-diaries/${form.index}`, {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json'
            },
  ...
  const fetchCompletedDiaries = async () => {
    try {
      const response = await fetch('http://192.168.5.119:3001/api/complete-diaries');
      const data = await response.json();
      setCompletedDiaries(data);

      // Cũng refresh danh sách nhật ký chính
      const diaryResponse = await fetch('http://192.168.5.119:3001/api/crop-diaries');
...
const handleStageFormSubmit = async (e) => {
  ...
  let imageFile = stageForm.imageProd;
  const formData = new FormData();
  ...
  try {
    ...
    await fetch(`http://192.168.5.119:3001/api/crop-diaries/${diary.index}`, {
      method: 'PUT',
                            ...
                            <button
                              ...
                              onClick={async () => {
                                try {
                                  ...
                                  const traceRes = await fetch('http://192.168.5.119:3001/api/trace', {
                                    method: 'POST',
                                    headers: {
                                      'Content-Type': 'application/json'
                                    },
                                  ...
                                  const updateData = {
                                    ...diary,
                                    tracingStatus: 'Đã truy xuất'
                                  };

                                  const statusRes = await fetch(`http://192.168.5.119:3001/api/complete-diaries/${diary.index}`, {
                                    method: 'PUT', 
                                    headers: {
                                      'Content-Type': 'application/json'
                                    },
                                  ...
                                  if (!statusRes.ok) {
                                    ...
                                    await fetch(`http://192.168.5.119:3001/api/trace/${diary.index}`, {
                                      method: 'DELETE'
                                    });
                            ...
                            <button
                              ...
                              onClick={async () => {
                                try {
                                  ...
                                  const updateRes = await fetch(`http://192.168.5.119:3001/api/complete-diaries/${diary.index}`, {
                                    method: 'PUT',
                                    headers: { 'Content-Type': 'application/json' },
                                    body: JSON.stringify({ ...diary, tracingStatus: 'Đã truy xuất' })
                                  });

                                  if (!updateRes.ok ) {
                                    // Rollback nếu cập nhật trạng thái thất bại
                                    await fetch(`http://192.168.5.119:3001/api/trace/${diary.index}`, { method: 'DELETE' });
                                    throw new Error('Không thể cập nhật trạng thái');
                                  }
                ...
                {/* Tìm đoạn code xử lý nút xóa và sửa lại: */}
                  <button
                    ...
                    onClick={async () => {
                      try {
                        // Xóa nhật ký chính
                        const res = await fetch(`http://192.168.5.119:3001/api/crop-diaries/${diaryToDelete.index}`, { 
                          method: 'DELETE'
                        });
                        if (res.ok) {
                          setDiaryList(prevList => 
      ...
      <form
        onSubmit={async (e) => {
          ...
          try {
            ...
            // Gửi thêm outputQty lên backend
            const res = await fetch(`http://192.168.5.119:3001/api/crop-diaries/${diaryToConfirm.index}`, {
              method: 'PUT',
              headers: {
                'Content-Type': 'application/json'
              },
              body: JSON.stringify({
                stage: 'Hoàn thành',
                updates: updates,
                outputQty: outputQty // <-- Thêm trường này
              })
            });
              ...
              setDiaryList(prevList =>
                prevList.map(d =>
                  d.index === diaryToConfirm.index
                    ? { ...d, stage: 'Hoàn thành', updates, outputQty }
                    : d
                )
              );
