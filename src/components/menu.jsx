...
export const menuItems = [
  ...
  { id: 'users', label: 'Thêm tài khoản', icon: Users, adminOnly: true }
];

function SidebarMenu({ activeMenu, setActiveMenu, setActiveTab, sidebarCollapsed, user }) {
  return (
    <nav className="p-2 sticky top-0 z-30 bg-gray-800">
      {menuItems
        .filter(item =>
          !item.adminOnly ||
        ...
        .map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center space-x-3 px-3 py-2 rounded-lg mb-1 transition-colors ${activeMenu === item.id
